...
async function Start(key) {
    ClearButtons();
    document.body.addEventListener('mouseup', Start2, true);
}
async function Start2() {
    document.body.removeEventListener('mouseup', Start2, true);
    await SlowType("I'm here! I'm here!");
    await WaitThenType(500, "¶¶So sorry for the wait! It's been so long since a player has come here!");
    await WaitThenType(500, "¶Don't worry; everything's ready, I just need to put it where you can see it!");
    await WaitThenType(3000, "¶Dammit... I put it right here! Where did it go?");
    await timer(1000);
    ButtonAppear();
    await SlowType("¶¶Here, you can use this prototype for now. I promise I'll get you something better if I can find it.");
    await timer(1000);
    await WaitThenType(500, "¶What else do I need to give you...");
    await WaitThenType(1000, " Oh, right!");
    await timer(500);
    LeftAppear();
    await SlowType("¶¶There you go! I'll tell you everything you need to know, but that should handle the basics, so I can focus on the details.");
    await WaitThenType(500, "¶I had to delete a few buttons to make room, but I don't think we're going to run out any time soon.");
    await WaitThenType(1000, "¶The actual UI already fit perfectly, but apparently I just don't get to show it to you.");
    await WaitThenType(1000, "¶I bet Ash knows where it is...");
    await WaitThenType(500, " Whatever, I'll ask around later.");
    await WaitThenType(1500, "¶¶I should probably introduce myself.");
    await WaitThenType(500, "¶I'm the Engine! My job is to help players like you!");
    await WaitThenType(500, "¶It's... complicated, but it would be *really* hard to actually render everything here in a way you could actually understand. I can go into it if you want, but just trust me that text is the best way to translate what happens in here to you out there.");
    await WaitThenType (500, "¶So you can think of me as a narrator! I'll tell you everything you need to know, what's happening, where you are, all that kind of stuff!")
    ClearButtons();
...
async function Start3(key) {
    if (key == 0) {
        ...
        await timer(50);
        ClearButtons();
        await SlowType("¶¶Look at you, using buttons!");
        await WaitThenType(500, "¶It's hard to explain exactly without a *lot* of context, but think of it this way. If I show you a picture, at least the kind that you're used to, that means I have to show you *everything* in the picture. If there's a person in it, you'll see *exactly* what they look like, and everything in the picture has the same level of detail.");
        await WaitThenType(1000, "¶That's...");
        await WaitThenType(500, " not really an option, here.");
        await WaitThenType(1000, "¶But with words, or, on your end, text, that isn't a problem! I can tell you what's important, and give you all the details you need, but I can also leave out the stuff that isn't important, or that wouldn't make a lot of sense to you. And don't worry; we all agreed on that a long time ago, so no one's getting left out!¶");
        Start3(1);
    }
    if (key == 1) {
        await timer(50);
        ...
        await WaitThenType(500,"¶Anyway, sorry again about the UI. I'll see if anyone knows where it went.");
        await WaitThenType(500,"¶I should probably get you started...");
        await WaitThenType(1000,"¶¶Oh! Real quick, your avatar is pretty basic, so you don't have to customize anything there, but it would help to know what I should call you. Let's start with your name.");
    ...
    if (key == 3) {
        ...
        await SlowType("Great! That should be everything!");
        await WaitThenType(500, "¶Don't worry; the slow text is just for when we're talking like this. When I'm actually narrating you'll just see it all at once.");
        await WaitThenType(500, "¶So let's get you started! Like I said, it's been a long time since we had a player here, so people might be a little... excited. Try not to hold it against them.");
        await WaitThenType(500, "¶Ready?")
        buttons[0][0].update(Start3.bind(null, 4), "Start");
    }
    if (key == 4) {
        await timer(50);
        ClearButtons();
        await SlowType("¶¶Here we go!");
        await timer(1500);
